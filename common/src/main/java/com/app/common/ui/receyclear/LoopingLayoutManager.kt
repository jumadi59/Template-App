package com.app.common.ui.receyclear
...
class LoopingLayoutManager : LinearLayoutManager {
    /**
     ...
     */
    @JvmOverloads
    public constructor(context: Context, orientation: Int = RecyclerView.VERTICAL, reverseLayout: Boolean = false) : super(context, orientation, reverseLayout) {
    ...
    public override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        try {
            while (sizeFilled < size) {
                ...
                prevItem = item
                Log.e("TAG", "$sizeFilled $size ${item.size}")
                if (!(view.parent as ViewGroup).isVisible) break
            }
        } catch (e: Exception) {
     ...
     * visible.
     */
    public override fun findFirstVisibleItemPosition(): Int {
     ...
     */
    public override fun findFirstCompletelyVisibleItemPosition(): Int {
        var lowestIndex = Int.MAX_VALUE;
        for (i in 0 until childCount) {
            val view = getChildAt(i);
     ...
     * visible.
     */
    public override fun findLastVisibleItemPosition(): Int {
        var highestIndex = 0;
        for (i in 0 until childCount) {
            val view = getChildAt(i)
     ...
     */
    public override fun findLastCompletelyVisibleItemPosition(): Int {
        var highestIndex = 0;
        for (i in 0 until childCount) {
            val view = getChildAt(i)
